...
extern "C" __declspec(dllexport) void add_native_detour(rage::joaat_t script_hash, uint64_t native_hash, rage::scrNativeHandler detour)
{
	big::g_native_hooks->add_native_detour(script_hash, native_hash, detour);
}

extern "C" __declspec(dllexport) void hook_program(rage::scrProgram* scr_program)
{
